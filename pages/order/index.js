...
export const getStaticProps = async () => {
  ...
  const paymentTypes = await fetch(
    process.env.NEXT_PUBLIC_HOST + "/paymenttypes"
  ).then((res) => res.json());

  const deliverytypes = await fetch(
    process.env.NEXT_PUBLIC_HOST + "/deliverytype"
  ).then((res) => res.json());
...
const Order = ({ categories, paymentTypes, deliverytypes }) => {
  ...
  const [selectedPaymentType, setSelectedPaymentType] = React.useState(
    paymentTypes.length ? paymentTypes[0] : ""
  );
  const [selectedDeliveryType, setSelectedDeliveryType] = React.useState(
    paymentTypes.length ? deliverytypes[0] : ""
  );
  ...
  const handleCheckout = React.useCallback(async () => {
    ...
    const config = {
      ...
      headers: {
        ...
        Authorization:
          "Bearer " + localStorage.getItem(process.env.NEXT_PUBLIC_LS_TOKEN),
      },
      body,
    };
      ...
      <div className={styles.root}>
        ...
        <div className={styles.container}>
          ...
          <div className={styles.content}>
            ...
            {!user.isAuth && <div className={styles.error}>{error}</div>}
          </div>
        </div>
      </div>
    </LayoutController>
